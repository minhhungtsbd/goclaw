...
func builtinToolSeedData() []store.BuiltinToolDef {
	defs := []store.BuiltinToolDef{
		...
		{Name: "cloudmini_proxy_check", DisplayName: "Cloudmini Service Check", Description: "Check Cloudmini Proxy or VPS service details, expiry, plan, account email, or live GeoIP for one IP", Category: "web", Enabled: true,
		...
		{Name: "admin_handoff_status", DisplayName: "Admin Ticket Status", Description: "Verify the current status of a historical Admin ticket for the active customer conversation", Category: "messaging", Enabled: true},
