...
func TestCloudminiServicePreflightCallsServiceInfoBeforeLLM(t *testing.T) {
	...
	stage := NewCloudminiServicePreflightStage(&PipelineDeps{
		ExecuteToolCall: func(_ context.Context, _ *RunState, tc providers.ToolCall) ([]providers.Message, error) {
			...
			return []providers.Message{{Role: "tool", ToolCallID: tc.ID, Content: `{"plan":"PrivateV4"}`}}, nil
		},
	})
	if err := stage.Execute(context.Background(), state); err != nil {
		t.Fatalf("preflight: %v", err)
	}
	if len(calls) != 1 || calls[0].Name != cloudminiProxyCheckToolName || calls[0].Arguments["operation"] != "service_info" {
		t.Fatalf("calls = %#v", calls)
	}
	if got := state.Messages.Pending(); len(got) != 2 || got[0].Role != "assistant" || got[1].Role != "tool" {
