...
func NewAGYHostHandlerFromEnv() (*AGYHostHandler, error) {
	...
	if base == "" || token == "" {
		return nil, errors.New("GOCLAW_AGY_HOST_BRIDGE_URL and GOCLAW_AGY_HOST_TOKEN must both be set")
	}
	u, err := url.Parse(base)
	if err != nil || (u.Scheme != "http" && u.Scheme != "https") || u.Host == "" {
		return nil, errors.New("invalid GOCLAW_AGY_HOST_BRIDGE_URL")
	}
...
func (h *AGYHostHandler) profile(r *http.Request) (string, bool) {
	p := r.PathValue("profile")
	return p, agyHostProfileRE.MatchString(p)
}
...
func (h *AGYHostHandler) bridgeRequest(r *http.Request, suffix string) (*http.Request, error) {
	...
	if err == nil {
		req.Header.Set("Authorization", "Bearer "+h.bridgeToken)
	}
	return req, err
}
...
func (h *AGYHostHandler) proxyJSON(w http.ResponseWriter, r *http.Request, endpoint string) {
	...
	if err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]string{"error": "AGY host bridge unavailable"})
		return
	}
...
func (h *AGYHostHandler) handleStatus(w http.ResponseWriter, r *http.Request) {
	if p, ok := h.profile(r); ok {
		h.proxyJSON(w, r, "/v1/profiles/"+p+"/status")
	} else {
		http.Error(w, "invalid profile", http.StatusBadRequest)
	}
}
func (h *AGYHostHandler) handleModels(w http.ResponseWriter, r *http.Request) {
	if p, ok := h.profile(r); ok {
		h.proxyJSON(w, r, "/v1/profiles/"+p+"/models")
	} else {
		http.Error(w, "invalid profile", http.StatusBadRequest)
	}
}
...
func (h *AGYHostHandler) handleTicket(w http.ResponseWriter, r *http.Request) {
	...
	h.tickets[ticket] = agyHostTicket{profile: p, expiresAt: time.Now().Add(agyHostTicketTTL)}
	h.mu.Unlock()
...
func (h *AGYHostHandler) consumeTicket(profile, ticket string) bool {
	...
	defer h.mu.Unlock()
	h.purgeExpiredLocked(time.Now())
	value, ok := h.tickets[ticket]
	if !ok || value.profile != profile {
		return false
	}
	delete(h.tickets, ticket)
	return true
}
func (h *AGYHostHandler) purgeExpiredLocked(now time.Time) {
	for key, value := range h.tickets {
...
func (h *AGYHostHandler) handleTerminal(w http.ResponseWriter, r *http.Request) {
	p, ok := h.profile(r)
	if !ok || !h.consumeTicket(p, r.URL.Query().Get("ticket")) {
	...
	defer client.Close()
	bridge, err := url.Parse(h.bridgeURL)
	if err != nil {
		return
	}
	if bridge.Scheme == "https" {
		bridge.Scheme = "wss"
	} else {
		bridge.Scheme = "ws"
	}
	...
	if err != nil {
		_ = client.WriteMessage(websocket.TextMessage, []byte("Unable to connect to AGY host bridge."))
		return
	}
	defer server.Close()
