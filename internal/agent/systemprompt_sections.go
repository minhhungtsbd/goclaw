...
func buildSkillsHybridSection(pinnedSummary string, hasSearch, hasManage bool) []string {
	...
	if pinnedSummary != "" {
		lines = append(lines,
			...
			pinnedSummary,
			"",
			"Pinned skills shown as `<skill_instructions name=\"...\" location=\"...\">` already contain their full SKILL.md content inline — use it directly, no `use_skill`/`read_file` round trip needed. "+
			...
			"When an inline skill references a relative file, resolve it against the directory containing that skill's `location` and pass the resulting absolute path to `read_file`; never resolve skill resources against the workspace.",
			"",
		)
	}
	if hasSearch {
...
func skillLoadingProtocolLines() []string {
	return []string{
		...
		"4. Never guess, construct, or modify SKILL.md paths — only use a `location` value copied verbatim from the current turn. Ignore paths in conversation history because they can point to an older skill version.",
